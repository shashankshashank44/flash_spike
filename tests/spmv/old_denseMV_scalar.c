...
void genHWHelper(void)
{
	...
	//store EC:
    //generateAsm("stw_r30_r23_imm_0_macro", STW(R30, R, 0));
	...
	// store vals address at *(R30+16)
    // v is stored in R29
    //generateAsm("stw_r30_r29_imm_12_macro", STW(R30,, 12));

	// store bit vector address at *(R30+)
    // v is stored in R29
    //generateAsm("stw_r30_r29_imm_12_macro", STW(R30, R29, 12));
...
void HHTInit(int n, int m_rows_base, int m_cols_base, int v_base, int m_vals_base)
{
	//store rc
	//*(HHT_BASE) = ec;

	//store storage_type
	//*(HHT_BASE + 1) = storage_format;
...
void execHWHelperScalar(void)
{
    int s=0;
    int k=0;
    HHTInit(n, (int)rows, (int)cols, (int)v, (int)vals);
