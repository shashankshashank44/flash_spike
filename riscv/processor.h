...
public:
  ...
  int isPrefetchRange(reg_t addr);
  ...
  uint64_t cycles;
  uint64_t clk;
  uint64_t buffer_wait_cycles = 0;		//cycles CPU waitied for buffer to send v[]
  uint64_t memory_wait_cycles = 0;		//cycles CPU waited for all memory requests
  float proc_speed;  
  void tick();
  void l2_tick();
  bool record_clk = 0;
...
private:
  ...
  unsigned long PREFETCH_BASE;
  unsigned long PREFETCH_RANGE;
  extension_t* ext;
