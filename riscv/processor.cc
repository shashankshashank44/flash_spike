  ...
  halt_on_reset(halt_on_reset), last_pc(1), executions(1), num_insn(0)
{
  ...
  BUFFER_RANGE = (8*1024);
  PREFETCH_BASE = (3*1024*1024*1024) + (1024 * 1024);
  PREFETCH_RANGE = (3*1024);
  bc = new buffer_controller_t(mmu, this, VU.VLEN/8);
  initialize_delays(timing);
  cycles = 0;
  clk = 0;
  //wait_cycles = 0;
  proc_speed = 10; //if 10, 2GHz; 800 MHz - mem ; 1000 MHz - cpu; 4:5-ratio of mem:pro
...
int processor_t::isPrefetchRange(reg_t addr){
	if((addr >= PREFETCH_BASE) && (addr < (PREFETCH_BASE+PREFETCH_RANGE))) return 1;
	return 0;
}

void processor_t::l2_tick()
{
	if(L2_req){
		L2_wait--;	
		if(L2_wait == 0)
	...
	//clk = clk + mmu->L2_hitdelay;
}

void processor_t::tick()
{
	clk++;
	//std::cout<<"clk:"<<clk<<"\n";
	//cycles stores advaced delays, clk stores present clock. if cycles is less than clk, make sure it 		//is in line with the clk. (this seems more logical)
	if(cycles < clk)
		cycles = clk;
	...
	//if(record_clk)
		//printf("clk:%ld %ld %ld %ld %ld\n",clk, cycles,bc->available_clk, bc->b[0].buffer_available_clk, bc->b[1].buffer_available_clk);
	//fflush(stdout);
	
	//DRAM tick
	//if( (((int)mmu->md->mem_speed * clk)/(int)proc_speed) != mmu->md->clk)		//assuming CPU is twice as fast as memory
	//	mmu->md->tick();

	//Flash tick
	if( (((int)mmu->fm->flash_speed * clk)/(int)proc_speed) != mmu->fm->clk )	
		mmu->fm->tick();

	//Buffer controller tick
	//std::cout<<clk<<" "<<bc->available_clk<<" "<<bc->start<<" "<<bc->under_process<<"\n";
	if(!bc->busy && bc->start /*&& !bc->under_process*/ )    //once read32 function called, bc->available_clk is updated based on the memory request processing delay. untill that delay bc->read32 is not called
	{
		//bc->fill_buffer();
		if(bc->op == 1)
            bc->fill_buffer_csr_noexp();
        else if(bc->op == 0){
            switch(bc->format){
                case 0:bc->fill_buffer_csr_exp(); break;
                case 1:bc->fill_buffer_bv_exp(); break;
                case 2:bc->fill_buffer_rl_exp(); break;
            }
        }
	}
}
...
processor_t::~processor_t()
{
  ...
  fprintf(stderr, "Number of Cycles taken: %ld\n", cycles);
  fprintf(stderr, "Buffer wait cycles: %ld\n", buffer_wait_cycles);
  fprintf(stderr, "Memory wait cycles: %ld\n", memory_wait_cycles);
