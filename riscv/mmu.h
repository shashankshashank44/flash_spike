...
public:
  ...
  #define load_func(type) \
    inline type##_t load_##type(reg_t addr) { \
	  bool req_host;/*0 is for CPU, 1 is for buffC*/\
      if (proc && proc->isBCRange(addr)) {\
		long temp_wait_1, temp_wait_2 = 0;\
		/*std::cout<<"buffer_access:"<<proc->bc->b[proc->bc->read_buffer_id].buffer_full<<" "<<proc->clk<<"\n";*/\
		if(!proc->bc->b[proc->bc->read_buffer_id].buffer_full){/*checking if buffer is full to process*/\
			/*std::cout<<"buffer_not_full\n";*/\
			/*This entire time CPU is waiting*/\
			temp_wait_1 = proc->clk;\
			while(!proc->bc->b[proc->bc->read_buffer_id].buffer_full){proc->tick();}\
			temp_wait_2 = proc->clk;\
			proc->buffer_wait_cycles += temp_wait_2 - temp_wait_1;\
		}\
		/*debug*/\
		/*debug*/\
		temp_wait_1 = proc->clk;\
		while(proc->clk < proc->bc->b[proc->bc->read_buffer_id].buffer_available_clk)/*loop through buffer delays are satisfied*/\
		{\
			/*This entire time CPU is waiting*/\
			/*std::cout<<"waiting_for_bc_clk\n";*/\
			proc->tick();\
		}\
		temp_wait_2 = proc->clk;\
		proc->buffer_wait_cycles += temp_wait_2 - temp_wait_1;\
		type##_t data = (type##_t)proc->bc->read_buffer();\
        return from_le(data);\
      }\
	  req_host = 0;\
      ...
      type##_t res; \
      load_slow_path(proc, addr, sizeof(type##_t), (uint8_t*)&res, req_host); \
      return from_le(res); \
    }
...
# define WRITE_MEM(addr, val, size) ({ \
  ...
  #define store_func(type) \
    void store_##type(reg_t addr, type##_t val) { \
	  if(proc && proc->isPrefetchRange(addr)) {\
		/*std::cout<<"prefetching:"<<addr<<"\n";*/\
		pf->initialize(addr-proc->PREFETCH_BASE, (uint8_t*)&val);	\
		return; \
	  }\
      if (proc && proc->isBCRange(addr)) {\
  ...
  void load_slow_path(processor_t* proc, reg_t addr, reg_t len, uint8_t* bytes, bool req_host);
  ...
  uint64_t temp_clk = 0;	//copy clock to check vector instruction loads
  int L2_hitdelay = 5;
  int log_lines = 5;
  bool req_satisfied;
  reg_t pf_req_wait = 0;
  uint64_t prefetch_wait = 0;

  void tick();
  processor_t* proc;
  reg_t translate(reg_t addr, reg_t len, access_type type);		//moved from private section
...
private:
  ...
  //reg_t translate(reg_t addr, reg_t len, access_type type);		//moved to public section
