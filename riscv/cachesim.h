...
class cache_sim_t
{
 public:
  ...
  int access(uint64_t addr, size_t bytes, bool store);
  void print_stats();
...
class icache_sim_t : public cache_memtracer_t
{
 public:
  ...
  {
    int miss = 0;
    if (type == FETCH) miss = cache->access(addr, bytes, false);
    return miss;
  }
};
...
class dcache_sim_t : public cache_memtracer_t
{
 public:
  ...
  {
    int L2_miss = 0;
    if (type == LOAD || type == STORE) L2_miss = cache->access(addr, bytes, type == STORE);
    return L2_miss;
  }
};
