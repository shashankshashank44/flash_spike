...
int cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  {
	//std::cout<<"hit:"<<name<<" addr:"<<addr<<"\n";
    if (store)
      *hit_way |= DIRTY;
	/*if(name.compare("L2$") == 0)
		return 2;			// 2 indicates L2 hit and L1 miss
	else
    	return 0;           //0 indicates L1 hit*/
  ...
  if (miss_handler){
	//std::cout<<"miss:"<<name<<"\n";
    returns = miss_handler->access(addr & ~(linesz-1), linesz, false);
  }

  if (store)
    *check_tag(addr) |= DIRTY;
  
  //std::cout<<"miss_name:"<<name<<" "<<returns<<"\n"; 
  //std::cout<<"miss:"<<name<<" addr:"<<addr<<"\n"; 
  /*if(name.compare("L2$") == 0)		//if L2 miss, return 1
  {
    return 1; 
  }
